...
class TC_Bisect < Test::Unit::TestCase
	...
	def test_insort_left
		list = [1, 2, 3, 3]
		assert_equal([1, 2, 2, 3, 3], Bisect.insort_left(list, 2))
		
		list = [[1, 2], [2, 3]].freeze
		assert_equal([['a', 2], [1, 2], [2, 3]], Bisect.insort_left(list.dup, ['a', 2]){|a, b| a[1] <=> b[1]})
		assert_equal([[1, 2], ['a', 3], [2, 3]], Bisect.insort_left(list.dup, ['a', 3]){|a, b| a[1] <=> b[1]})
		assert_equal([[1, 2], [2, 3], ['a', 4]], Bisect.insort_left(list.dup, ['a', 4]){|a, b| a[1] <=> b[1]})
	end
end
